...
def format_clips_v3(clips):
    ...
    if has_rp_n3:
        findings.append("tributary reflux")
    prompt = "Patient with varicose veins. Duplex: " + ", ".join(findings) + ".\n\nProvide only the CHIVA type classification. Do not repeat the question. Answer only with the type."
    return prompt
...
for idx, test in enumerate(test_cases, 1):
    ...
    v3_clips = format_clips_v3(clips)
    ...
    def qwen_run(prompt):
        try:
            ...
            # Strip repeated prompt if present (training data artifact)
            for line in prompt.split('\n'):
                if line.strip() and response.startswith(line.strip()):
                    response = response[len(line.strip()):].lstrip()
                    break
    ...
    qwen_c_v3 = qwen_run(v3_clips)

    # Ligation
    qwen_l_v1 = qwen_run(v1_clips + "\nProvide ligation planning strategy.")
    qwen_l_v2 = qwen_run(v2_clips + "\nProvide ligation planning strategy.")
    qwen_l_v3 = qwen_run(v3_clips.replace("classification", "planning strategy") + "\nProvide only the ligation planning strategy. Do not repeat the question.")

    results['shunt_classification'].append({
        'test': test['name'],
        'qwen_v1': qwen_c_v1,
        'qwen_v2': qwen_c_v2,
        'qwen_v3_explicit': qwen_c_v3,
    })
...
for result in results['shunt_classification']:
    ...
    doc.add_heading("V3 Query (Explicit: Answer Only, No Repetition)", level=3)
    doc.add_paragraph(result['qwen_v3_explicit'])
