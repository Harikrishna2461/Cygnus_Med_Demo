...
def run_guidance_crew(
    ...
    task3 = Task(
        description=(
            ...
            "  If all three are in the list, cycle back to whichever was suggested least recently.\n\n"
            ...
            "  FOR STATE E (Q1-Q4 confirmed):\n"
            "  CASE E1 — No elimTest in any clip: Output ONLY: "
            f"'STATE E1. Probe at {current_zone}. No routing needed — elimination test pending.' "
            ...
            "  CASE E2 — elimTest IS recorded on at least one clip: Circuit is fully classified. "
            "  The surgeon is now scanning for additional shunts. "
            "  Route to the most diagnostically useful unvisited zone: "
            "  SPJ/popliteal fossa first (check for SSV circuit), then upper thigh, then calf. "
            "  DO NOT route to SFJ/groin — the SFJ entry has already been mapped and classified. "
            f"  Output: 'STATE E2. Circuit classified. Route to [zone].' \n\n"
    ...
    task4 = Task(
        description=(
            ...
            "    'GSV at SFJ junction in groin crease'  (ONLY when Circuit Analyst says SFJ)\n"
            "    NEVER say just 'GSV' or just 'SPJ junction' without naming where on the leg.\n\n"
            "ANTI-DEFAULT RULE: Do NOT route to SFJ/groin as a default or fallback. "
            "SFJ is only valid when the Circuit Analyst explicitly says to examine at SFJ "
            "in STATE A. If the Circuit Analyst mentions SFJ only as a historical finding "
            "(e.g., 'EP was found at SFJ'), that is NOT a routing instruction — "
            "route to whatever zone the Circuit Analyst tells you to go next.\n\n"
