...
_SYSTEM_PROMPT = (
    ...
    "  5. SCAN THE ENTIRE IMAGE including corners, edges, and the area ABOVE the fascia lines.\n"
    "     N3 labels often appear in the TOP PORTION of the frame, above the fascia lines, sometimes\n"
    "     near the top-left or top-right corners. Do NOT stop after finding N2 — keep scanning.\n"
    "  6. N3 ovals may be smaller or partially at the image edge — still report them if visible.\n\n"
    "frame_note: 1 natural sentence describing ALL labeled structures you see anywhere in the image "
    "(e.g. 'Yellow fascia lines with N2 between them and N3 oval visible in top-right.' or "
    "'Yellow fascia lines visible with no labeled vessel.' or "
    "'Oval labeled GSV_DISTAL and TRIBUTARY, no fascia lines.')"
)
...
def _build_user_prompt(region: str, leg: str) -> str:
    ...
    return (
        ...
        f"Step 1 — systematically scan the ENTIRE image from top-left to bottom-right, "
        f"paying special attention to ALL four corners and edges. READ EVERY TEXT LABEL "
        f"printed near an oval/circle outline anywhere in the frame.\n"
        f"  IMPORTANT for Style A frames: N3 labels often appear ABOVE the fascia lines "
        f"in the TOP section of the image — look there explicitly before concluding no N3 is present.\n"
        ...
        f"Report EVERY labeled vessel — if multiple ovals with different labels, add all of them.\n"
