...
class Florence2Processor(ProcessorMixin):
    ...
    def __init__(
        ...
        tokens_to_add = {
                'additional_special_tokens': \
                    (tokenizer.additional_special_tokens if hasattr(tokenizer, 'additional_special_tokens') else getattr(tokenizer, 'special_tokens_map', {}).get('additional_special_tokens', [])) + \
                    ['<od>', '</od>', '<ocr>', '</ocr>'] + \
    ...
    def __call__(
        ...
        _img_kwargs = {'return_tensors': return_tensors}
        if do_resize is not None:    _img_kwargs['do_resize']          = do_resize
        if do_normalize is not None: _img_kwargs['do_normalize']        = do_normalize
        if image_mean is not None:   _img_kwargs['image_mean']          = image_mean
        if image_std is not None:    _img_kwargs['image_std']           = image_std
        if input_data_format is not None: _img_kwargs['input_data_format'] = input_data_format
        if data_format is not None:  _img_kwargs['data_format']         = data_format
        if resample is not None:     _img_kwargs['resample']            = resample
        if do_convert_rgb is not None: _img_kwargs['do_convert_rgb']    = do_convert_rgb
        pixel_values = self.image_processor(images, **_img_kwargs)["pixel_values"]
