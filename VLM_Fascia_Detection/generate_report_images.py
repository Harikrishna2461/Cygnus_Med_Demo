...
def make_overlay(img_rgb_np, fascia_prob, vein_prob,
                 fascia_thresh=0.35, vein_thresh=0.5, alpha=0.55):
    """Blend fascia (cyan) + vein (green) masks onto the image."""
    ...
    fascia_mask = fp > fascia_thresh
    vein_mask   = vp > vein_thresh

    for mask, col in [(fascia_mask, FASCIA_COL), (vein_mask, VEIN_COL)]:
        if mask.any():
...
def add_label_bar(img_np, label, font_size=22):
    """Add a white label bar at the top of the image."""
    h, w = img_np.shape[:2]
    bar_h = 36
    bar = np.ones((bar_h, w, 3), dtype=np.uint8) * 30  # dark bar
...
def make_panel(img_pil, baseline_f, baseline_v, ft_f, ft_v, panel_size=400):
    """Create a horizontal panel: Input | Baseline overlay | Fine-tuned overlay."""
    img_np = np.array(img_pil)

    baseline_overlay = make_overlay(img_np, baseline_f, baseline_v)
    ft_overlay       = make_overlay(img_np, ft_f,       ft_v)

    # Resize all to panel_size × panel_size
    def resize(a):
        return np.array(Image.fromarray(a).resize((panel_size, panel_size), Image.BICUBIC))

    inp_r  = resize(img_np)
    base_r = resize(baseline_overlay)
    ft_r   = resize(ft_overlay)

    inp_r  = add_label_bar(inp_r,  'Input (1024×1024)')
    base_r = add_label_bar(base_r, 'Baseline (no fine-tuning)')
    ft_r   = add_label_bar(ft_r,   'Fine-tuned')

    # Add thin separator lines
    sep = np.ones((inp_r.shape[0], 4, 3), dtype=np.uint8) * 180
    panel = np.hstack([inp_r, sep, base_r, sep, ft_r])
...
def add_panels_to_doc(panels, section_title, section_body):
    ...
    PANEL_W = Inches(6.3)   # fits within 1-inch margins on 8.5" page
...
add_body(
    'Each panel shows three views of the same ultrasound frame: the preprocessed input '
    '(letterboxed to 1024×1024 to match training format), the baseline BiomedParse output '
    '(pretrained weights, no fine-tuning), and the fine-tuned model output. '
    'Cyan overlay = fascia prediction; green overlay = vein prediction.'
)
