...
def _prompt_verify(img_w: int, img_h: int, hint: tuple) -> str:
    ...
    return (
        ...
        "Check whether this box correctly isolates ONLY the ultrasound scan content "
        "(grayscale speckle pattern) while excluding ALL of the following: "
        "UI chrome, parameter panels, depth/gain rulers, logos, menus, "
        "thin dark border strips at any edge, and small probe-orientation markers "
        "(letters such as 'Q', 'L', 'R', 'N' that appear at scan-area corners). "
        "If the box is correct, return it unchanged. "
        "If it includes any of the above, return the tightest corrected box. "
        "Reply with ONLY this JSON and nothing else: "
        '{"x1": <int>, "y1": <int>, "x2": <int>, "y2": <int>}'
    )
