...
class Qwen2VL_MOE_Config(Qwen2VLConfig):
    ...
    def __init__(self,moe_enable=True,moe_mode='sparse',moe_layers_idx=None,ep_size=1,top_k_experts=2,capacity_factor=1.5,
        ...
        self.shared_expert_intermediate_size = 5632
        self.num_experts = 4
...
class VisionAttention(nn.Module):
    ...
    def forward(
        ...
        # self.att_weight = attn_weights.detach().cpu()######todo##########修改之后的
...
class Qwen2VLMOEForConditionalGeneration(Qwen2VLPreTrainedModel, GenerationMixin):
    ...
    def initialize_moe_modules(self, checkpoint_path):
        state_dict_all={}
        for filename in os.listdir(checkpoint_path):
            if filename.endswith(".safetensors"):
                file_path = os.path.join(checkpoint_path, filename)
                print(f"Loading weights from {file_path} to initialize MOE")
        ...
        for name, param in self.model.named_parameters():
            if 'gate_proj' in name or 'up_proj' in name or 'down_proj' in name:
                ...
                weight_name = f'model.layers.{name_list[1]}.mlp.{name_list[-2]}.weight'
    ...
    def forward(
        ...
        #############################todo
        aux_loss = None
        output_router_logits = False###todo
        if output_router_logits:
