...
from collators.datacollator_apply_chat_template import CustomDataCollatorApplyChatTemplate, InternVLProcessor_Custom,CustomDataCollatorApplyChatTemplateV2
from arguments_sft import ModelArguments, DataArguments, TrainingArguments
from collators.datasetV2 import LazySupervisedDatasetV2
...
def train():
    ...
    if model_args.model_id == 'lingshu_with_dinov3':
        ...
        default_assistant_start_tokens = "ASSISTANT:"
        default_assistant_end_tokens = "</s>"
    elif model_args.model_id == 'Qwen2_VL':  # 2B and 7B
    ...
    if model_args.training_stage == 'stage1':
        ...
        if model_args.use_peft: # 只有在启用 LoRA 时才进行 LoRA 配置
            rank0_print("Training lora parameters.")
            model.requires_grad_(False)
            named_modules = {n: m for n, m in model.named_modules()}
            lora_modules = []
            if model_args.train_vision_encoder_lora:
                rank0_print("LoRA for vision encoder enabled...")
                lora_modules.extend(find_all_linear_names(named_modules, model_args.lora_vision_encoder_keys))
            if model_args.train_llm_lora:
                rank0_print("LoRA for LLM enabled...")
                lora_modules.extend(find_all_linear_names(named_modules, model_args.lora_llm_keys))
            if model_args.train_vision_projector_lora:
                rank0_print("LoRA for Projector enabled...")
                lora_modules.extend(find_all_linear_names(named_modules, model_args.lora_vision_projector_keys))
            ...
            if model_args.load_in_4bit or model_args.load_in_8bit:
                rank0_print(f"Preparing model for {('4bit' if model_args.load_in_4bit else '8bit')} quantization ...")
    ...
    train_dataset = LazySupervisedDatasetV2(
    ...
    if data_args.eval_data_path:
        eval_dataset = LazySupervisedDatasetV2(
    ...
    # data collator
    data_collator = CustomDataCollatorApplyChatTemplateV2(model_args=model_args,training_args=training_args,
