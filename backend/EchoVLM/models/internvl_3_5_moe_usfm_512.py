...
from .dinov3 import DINOv3ViTModel, DINOv3ViTConfig
from .usfm.modeling_usfm import USFM_VisionTransformer

class InternVL_MOE_Config(InternVLConfig):

    def __init__(self,fusion_mlp_config= None,**kwargs):
        ...
        self.dinov3_config = DINOv3ViTConfig.from_pretrained('/data/scy/SCY/Model_weights/dinov3-vitl16-pretrain-lvd1689m')
        if fusion_mlp_config is None:
            fusion_mlp_config = {"hidden_size": 3072, "intermediate_size": 1024,"hidden_act":"gelu","usfm_projector_in":768,"usfm_projector_out":1024}
        self.fusion_mlp_config = fusion_mlp_config
        a=0
        # self.lora = {}
...
class FusionMLP(nn.Module):
    def __init__(self, config, bias: bool = False):
        ...
        self.down_proj = nn.Linear(self.intermediate_size, self.intermediate_size, bias=bias)
...
class InternVLModel(InternVLPreTrainedModel):
    ...
    def __init__(self, config: InternVL_MOE_Config):
        super().__init__(config)
        ...
        self.fusion_projector = FusionMLP(config.fusion_mlp_config)
        # self.usfm_projector = nn.Linear(config.fusion_mlp_config["usfm_projector_in"], config.fusion_mlp_config["usfm_projector_out"], bias=False)
        self.post_init()
    ...
    def get_dinov3_and_usfm_image_features(self,dinov3_pixel_values: Optional[torch.Tensor] = None,#TODO
                                  ):
        # stack_dinov3_pixel_values = torch.stack(dinov3_pixel_values)
        usfm_vision_features = self.usfm(dinov3_pixel_values).last_hidden_state[:,5:,:]
        # usfm_vision_features = self.usfm_projector(usfm_vision_features)
        dinov3_vision_features = self.dinov3(dinov3_pixel_values).last_hidden_state[:,5:,:]
        return dinov3_vision_features,usfm_vision_features

    def fusion(self,dinov3_pixel_values,vision_features,image_num_patches_indices):
        global_indices = image_num_patches_indices - 1  # [B]
        clip_vision_features = vision_features[global_indices]
        dinov3_vision_features,usfm_vision_features = self.get_dinov3_and_usfm_image_features(dinov3_pixel_values)
        concat_feat = torch.cat([dinov3_vision_features,usfm_vision_features, clip_vision_features], dim=-1)
        fused_feat = self.fusion_projector(concat_feat)
        vision_features.index_copy_(0, global_indices, fused_feat)
    ...
    def get_image_features(
        ...
        vision_feature_select_strategy: Optional[str] = None,
            image_num_patches_indices: Optional[torch.Tensor] = None,#TODO
            dinov3_pixel_values: Optional[torch.Tensor] = None,#TODO
        **kwargs,
    ):
        """
        ...
        # dinov3_vision_features = self.get_dinov3_and_usfm_image_features(dinov3_pixel_values=dinov3_pixel_values)
        vision_features = self.fusion(dinov3_pixel_values,vision_features,image_num_patches_indices)
    ...
    def forward(
        ...
        if pixel_values is not None:
            image_features = self.get_image_features(
                ...
                image_num_patches_indices=image_num_patches_indices,#TODO
                dinov3_pixel_values=dinov3_pixel_values,  # TODO
            )
...
class InternVL_MOE_USFM_512_ForConditionalGeneration(InternVLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            image_num_patches_indices=image_num_patches_indices,
...
__all__ = [
    ...
    "InternVLModel",
    "InternVL_MOE_USFM_512_ForConditionalGeneration",
]
