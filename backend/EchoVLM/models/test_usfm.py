...
from usfm.modeling_usfm import USFM_VisionTransformer
import torch.nn as nn

weights_path = Path("/data/scy/SCY/Model_weights/USFM_latest.pth")

# 2. 把权重加载到 CPU，再转到目标设备（避免 GPU 编号问题）
state_dict = torch.load(weights_path, map_location="cpu")
for key in state_dict.keys():
    print(key)
model = USFM_VisionTransformer(norm_layer=partial(nn.LayerNorm, eps=1e-6))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pytorch_dump_folder_path", default='/data/scy/SCY/SonoVLM_V2/usfm_vit', type=str, help="Path to the output PyTorch model directory."
    )
